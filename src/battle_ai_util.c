...
bool32 ShouldUseZMove(u32 battlerAtk, u32 battlerDef, u32 chosenMove)
{
    ...
    if (IsViableZMove(battlerAtk, chosenMove))
    {
        ...
        case EFFECT_FILLET_AWAY:
            ...
            break;
        case EFFECT_PROTECT:
            if (HasDamagingMoveOfType(battlerAtk, GetMoveType(GetMoveType(chosenMove))))
                return FALSE;
            else
                isEager = TRUE;
            break;
        ...
        case EFFECT_TRANSFORM:
            if (IsBattlerTrapped(battlerDef, battlerAtk) && !HasDamagingMoveOfType(battlerDef, GetMoveType(GetMoveType(chosenMove))))
                return TRUE;
            if (isSlower)
                isEager = TRUE;
            break;
